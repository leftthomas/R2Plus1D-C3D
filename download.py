...
def download_clip_wrapper(row, label_to_dir, trim_format, index):
    ...
    if os.path.exists(output_filename):
        print('Index:%-16s Clip-ID:%-22s Status:%-50s' % (index, clip_id, 'Exists'))
    else:
        downloaded, log = download_clip(row['video-id'], output_filename, row['start-time'], row['end-time'])
        if downloaded:
            print('Index:%-16s Clip-ID:%-22s Status:%-50s' % (index, clip_id, 'Downloaded'))
        else:
            print('Index:%-16s Clip-ID:%-22s Status:%-50s' % (index, clip_id, log.strip().decode('utf-8')))
