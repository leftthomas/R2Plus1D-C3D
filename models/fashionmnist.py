...
class FashionMNISTCapsuleNet(nn.Module):
    def __init__(self):
        ...
        self.features = nn.Sequential(
            CapsuleConv2d(in_channels=1, out_channels=16, kernel_size=7, in_length=1, out_length=8, stride=2,
                          padding=0),
            nn.BatchNorm2d(num_features=16),
            nn.ReLU(inplace=True),
            CapsuleConv2d(in_channels=16, out_channels=32, kernel_size=3, in_length=8, out_length=self.out_length,
                          stride=2,
                          padding=0),
