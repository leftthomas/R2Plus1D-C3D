...
class MNISTCapsuleNet(nn.Module):
    def __init__(self):
        ...
        self.features = nn.Sequential(
            CapsuleConv2d(in_channels=1, out_channels=4, kernel_size=5, in_length=1, out_length=2, stride=1,
                          padding=2),
            nn.BatchNorm2d(num_features=4),
            nn.ReLU(inplace=True),
            CapsuleConv2d(in_channels=4, out_channels=8, kernel_size=3, in_length=2, out_length=2, stride=2,
                          padding=1),
        ...
        self.classifier = CapsuleLinear(in_capsules=7 * 7 * 16 // self.out_length, out_capsules=10,
