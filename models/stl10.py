...
class STL10CapsuleNet(nn.Module):
    def __init__(self, routing_type='sum'):
        ...
        self.features = nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            CapsuleConv2d(in_channels=128, out_channels=128, kernel_size=3, in_length=32,
                          out_length=self.features_out_length, stride=2, padding=1),
            nn.BatchNorm2d(num_features=128),
            nn.ReLU(inplace=True),

        )
        self.classifier = CapsuleLinear(in_capsules=12 * 12 * 128 // self.features_out_length, out_capsules=10,
                                        in_length=self.features_out_length, out_length=64, routing_type=routing_type)
