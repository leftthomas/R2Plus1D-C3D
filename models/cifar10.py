...
class CIFAR10CapsuleNet(nn.Module):
    def __init__(self):
        ...
        self.features = nn.Sequential(
            ...
            nn.BatchNorm2d(num_features=64),
            nn.ReLU(inplace=True),
            CapsuleConv2d(in_channels=64, out_channels=64, kernel_size=3, in_length=8, out_length=16, stride=1,
                          padding=1),
            nn.BatchNorm2d(num_features=64),
            nn.ReLU(inplace=True),
        ...
        self.classifier = CapsuleLinear(in_capsules=2 * 2 * 64 // self.out_length, out_capsules=10,
