...
class CIFAR10CapsuleNet(nn.Module):
    def __init__(self, num_iterations=3):
        ...
        self.down_block1 = nn.Sequential(nn.Conv2d(in_channels=3, out_channels=64, kernel_size=1, stride=2),
                                         nn.BatchNorm2d(num_features=64))
        ...
        self.down_block3 = nn.Sequential(nn.Conv2d(in_channels=128, out_channels=256, kernel_size=1, stride=2),
                                         nn.BatchNorm2d(num_features=256))
        self.down_block4 = nn.Sequential(nn.Conv2d(in_channels=256, out_channels=256, kernel_size=1, stride=2),
                                         nn.BatchNorm2d(num_features=256))
        self.relu = nn.ReLU(inplace=True)
        self.classifier = nn.Sequential(CapsuleLinear(in_capsules=256, out_capsules=128, in_length=4, out_length=8,
